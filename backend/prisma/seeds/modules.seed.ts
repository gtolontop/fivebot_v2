...
- \`/modlogs\` - View moderation history
    ...
    configSchema: JSON.stringify({
      ...
      banLogChannel: { type: 'channel', label: 'Ban Log Channel (optional, uses main log if not set)', required: false },
      kickLogChannel: { type: 'channel', label: 'Kick Log Channel (optional, uses main log if not set)', required: false },
      warnLogChannel: { type: 'channel', label: 'Warning Log Channel (optional, uses main log if not set)', required: false },
      autoDeleteInvites: { type: 'boolean', label: 'Auto-delete Discord invites', default: false },
      maxWarnings: { type: 'number', label: 'Max warnings before auto-action', default: 3, min: 1, max: 10 },
      warningAction: { type: 'select', label: 'Action after max warnings', options: ['NONE', 'KICK', 'BAN', 'TIMEOUT'], default: 'TIMEOUT' },
      timeoutDuration: { type: 'number', label: 'Default timeout duration (minutes)', default: 60, min: 1, max: 40320 },
      banDeleteDays: { type: 'number', label: 'Delete message history when banning (days)', default: 1, min: 0, max: 7 },
      requireReason: { type: 'boolean', label: 'Require reason for moderation actions', default: true },
      dmOnModeration: { type: 'boolean', label: 'DM users when moderated', default: true },
      moderatorRoles: { type: 'roles', label: 'Moderator Roles (who can use mod commands)', required: false, multiple: true },
    }),
