...
cd /d "%~dp0\backend"
node -e "const Redis = require('ioredis'); const r = new Redis({ host: '83.150.218.42', port: 40121, password: 'P@55w0rd' }); r.flushall().then(() => { console.log('✅ Redis cleared'); process.exit(0); }).catch((e) => { console.log('❌ Redis error:', e.message); process.exit(1); });" 2>nul || echo Redis clear skipped
echo Done!
timeout /t 1 /nobreak >nul
